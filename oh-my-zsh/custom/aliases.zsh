...
alias dlint='(mfiles | grep -v -e "^setup" -e Procfile -e configspec -e development.conf -e "^tests/" | xargs bin/pylint); (mfiles | grep -e "^tests/" | xargs bin/pylint | grep 'Unused')'
alias dpep8='mfiles | grep -v -e "^setup" -e Procfile -e configspec -e development.conf | xargs bin/pep8 -r'
